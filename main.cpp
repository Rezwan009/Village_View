...
void StartingText()
{
    ...
    for(int i = 0; text[i] != '\0'; i++)
    {
        ...
        sprintf(text,"  AHMED REZWAN",5.00,8.00);
    glColor3f(0, 0, 0);
    glRasterPos2f( -100 , 0 );
    for(int i = 0; text[i] != '\0'; i++)
    {
...
void handleKeypress(unsigned char key, int x, int y) {
	switch (key) {
	    case  'd':
           ...
           // sndPlaySound("F:\Graphics\Village_View",SND_ASYNC);
           // PlaySound(TEXT("bird-2.wav"), NULL, SND_ASYNC);
            PlaySound(TEXT("F:\\Graphics\\Village_View\\cartoon-birds-2_daniel-simion.wav"), NULL, SND_ASYNC);//|SND_FILENAME|SND_LOOP
            break;
        case 'r':
            glutDestroyWindow(1);
	        glutInitWindowSize(1240, 750);
            ...
            PlaySound(TEXT("F:\\Graphics\\Village_View\\Rain_Background-Mike_Koenig-1681389445.wav"), NULL, SND_ASYNC);
           // sndPlaySound("cricket.wav", SND_ASYNC);
            break;
         case 'n':
            glutDestroyWindow(1);
	        glutInitWindowSize(1240, 750);
           ...
           // sndPlaySound("cricket.wav",SND_ASYNC);
            PlaySound(TEXT("F:\\Graphics\\Village_View\\cricket.wav"), NULL, SND_ASYNC);
            glutPostRedisplay();
            break;
        case  'D':
	        glutDestroyWindow(1);
            ...
            glutDisplayFunc(display);
            //sndPlaySound("cricket.wav",SND_ASYNC);
            PlaySound(TEXT("F:\\Graphics\\Village_View\\cartoon-birds-2_daniel-simion.wav"), NULL, SND_ASYNC);
            break;
        case 'R':
            glutDestroyWindow(1);
	        glutInitWindowSize(1240, 750);
